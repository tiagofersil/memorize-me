...
def checkout():
    ...
    public_key = mp_service.get_public_key() if mp_service else os.getenv('PUBLIC_KEY')
...
def process_payment():
    ...
    try:
        ...
        if payment_data.get('token'):
            ...
            if payment_method_id and 'debit' in payment_method_id:
                result = mp_service.create_debit_card_payment(payment_data)
            else:
                result = mp_service.create_credit_card_payment(payment_data)
                
        elif payment_method_id == 'pix':
            # PIX
            result = mp_service.create_pix_payment(payment_data)
            
        elif payment_method_id in ['bolbradesco', 'boletofacil']:
            # Boleto
            result = mp_service.create_boleto_payment(payment_data)
        ...
        if result["success"]:
            ...
            formatted_payment = mp_service.format_payment_for_frontend(payment)
            return jsonify(formatted_payment)
        else:
    ...
    try:
        ...
        if result["success"]:
            ...
            if status == 'approved':
                ...
                send_payment_confirmation_email(result["payment_data"])
            elif status == 'rejected':
                # Pagamento rejeitado - enviar email de falha
                send_payment_failure_email(result["payment_data"])
...
@payments_bp.route('/api/payment_methods')
def get_payment_methods():
    ...
    try:
        ...
        if result["success"]:
            return jsonify(result["payment_methods"])
        else:
            return jsonify({"error": result["error"]}), 400
...
@payments_bp.route('/api/installments')
def get_installments():
    """API para obter opções de parcelamento"""
    try:
        ...
        if not payment_method_id or not amount:
            return jsonify({"error": "Parâmetros obrigatórios: payment_method_id, amount"}), 400
        ...
        result = mp_service.get_installments(payment_method_id, float(amount), issuer_id)
        
        if result["success"]:
            return jsonify(result["installments"])
        else:
            return jsonify({"error": result["error"]}), 400
